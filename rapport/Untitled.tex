...
Dans ce projet, on s'int\'{e}resse \`{a} compl\'{e}ter une grille de taille $M\times N$. Dans un premier temps, on mod\'{e}lisera ce probl\`{e}me comme un probl\`{e}me de satisfaction de contraintes et on d\'{e}veloppera plusieurs m\'{e}thodes diff\'{e}rentes de recherche d'une solution au probl\`{e}me CSP : arc consistance du graphes des contraintes ($AC3$), retour arrière chronologique ($RAC$) avec forward checking ($FC$), et conflict back-jumping ($CBJ$) avec $FC$ lui aussi. La grille peut \^{e}tre d\'{e}j\`{a} partiellement remplie par l'utilisateur.
...
Tout cela doit être implémenté de la manière la plus simple possible en machine. Voici le détail de ce qui a été effectué pour ce projet.
...
Il est d'ailleurs intéressant de noter, comme le montrent les résultats de la section suivante, que le $CBJ$ donne globalement une solution légèrement moins rapidement que $RAC$. La raison pour cela a été donnée dans le dernier paragraphe de la section précédente (\ref{RAC}).

Pour ces deux derniers algorithmes, une fonctionnalité permettant de choisir si l'on souhaite que les mots n'apparaissent qu'une seule fois dans la grille a été implémentée. Cette particularité est vérifiée au niveau du Forward Checking, qui supprime du domaine des variables de même taille le mot qui vient d'être instancié. Par la suite, cette fonctionnalité sera toujours appliquée. En effet, souvent une solution plus rapide sera donnée car les grilles sont assez symétriques, et un même mot, au début d'un domaine, peut être placé à plusieurs endroits. Les mots se positionnent d'ailleurs de telle sorte qu'ils forment des structures elles aussi symétriques, comme sur la figure suivante :

\begin{figure}[H]
	\begin{center}
		\includegraphics{Images/gridA_not_uniq.png}
		\caption{Grille A résolue sans la contrainte (3)}
	\end{center}
\end{figure}

\subsection{Exp\'{e}rimentation}
Les algorithmes ont été appliqués aux grilles données dans le sujet (notées A, B et C, de gauche à droite), ainsi qu'à quelques autres grilles récupérées du site :\\ \mbox{\url{http://puzzles.about.com/od/howtostutorials/ig/CrosswordGrids/}}, et à quelques instances générées aléatoirement. Les résultats sont stockés dans le dossier du projet :\\
...
On peut noter que le temps d'exécution augmente au début avec la taille du dictionnaire. Mais les deux dernier dictionnaires, assez conséquents de taille approximative 133000 mots et 135000 mots nous donnent tort. Il est probable que ces dictionnaires s'arrangent bien avec la grille. Il faut donc comparer avec une autre grille, qui nous donne des résultats plus ou moins similaires :
...
En chacune des feuilles de l'arbre, on a une solution possible qui correspond ou non \`{a} une solution admissible et on retient la meilleure solution obtenue, qui dans ce cas, sera la solution de poids maximum.\\
...
Si la borne inf\'{e}rieure \`{a} une feuille de l'arbre (c'est-à-dire apr\`{e}s avoir instancié les n variables de la grille) est sup\'{e}rieure \`{a} la valeur de la solution courante, on a trouv\'{e} une meilleure solution.
...
En effet, dans le meilleur des cas, la solution optimale est form\'{e}e de mots dont le poids est maximum dans le domaine de chaque variable de la grille, donc on obtient bien une borne sup\'{e}rieure \`{a} chaque sous-probl\`{e}me repr\'{e}sent\'{e} par les n\oe uds de l'arbre. Si cette borne sup\'{e}rieure est inf\'{e}rieure \`{a} la solution courante, il est inutile de continuer \`{a} explorer ce n\oe ud, puisque cette solution partielle ne pourrait pas conduire \`{a} am\'{e}liorer la solution courante.
...
Afin de construire un graphe montrant l'\'{e}volution du temps en fonction de la taille du dictionnaire, nous avons g\'{e}n\'{e}r\'{e} 10 instances de 6 dictionnaires contenant chacun 50, 300, 400, 500, 650 et 850 mots pour la grille A de taille $5 \times 5$. Nous avons aussi comparé les courbes lorsque les mots dans les grilles doivent être uniques ou pas.\\
...
Dans cette deuxi\`{e}me \'{e}tude, le temps d'ex\'{e}cution ne d\'{e}pend pas de la taille de la grille, mais du nombre de variables dans la grille. En effet, les grilles \`{a} 10 et 15 variables avaient toutes les deux la m\^{e}me taille ($7 \times 7$) mais ont des temps d'\'{e}xecution assez diff\'{e}rents dans le cas o\`{u} les mots ne peuvent apparaitre qu'une seule fois dans la grille. Il d\'{e}pend \'{e}galement du dictionnaire utilis\'{e}. En effet, dans notre exemple, pour des grilles \`{a} 12 variables, le temps d'ex\'{e}cution est sup\'{e}rieur aux temps d'ex\'{e}cution des grilles \`{a} 18 et 15 variables.
...
Dans cette partie, nous avons constitu\'{e} un dictionnaire avec des mots-cl\'{e}s utilis\'{e}s fr\'{e}quemment dans les cours du master ANDROIDE, tous de poids 1, et des mots suppl\'{e}mentaires de poids inf\'{e}rieur \`{a} 1. Avec une grille crée pour cette tâche, on obtient la solution suivante:\\

\begin{figure}[H]
	\begin{center}
		\includegraphics{Images/gridANDROIDE_full_with_dicoANDROIDE.png}
		\caption{Grille gridANDROIDE résolue par Branch \& Bound -- dictionnaire valué dicoANDROIDE}
		\label{gridANDROIDE_dicoANDROIDE}
	\end{center}
\end{figure}

Le temps d'ex\'{e}cution \'{e}tant \'{e}gal \`{a} environ 3.1 secondes sans lancer $AC3$ préalablement, et  légèrement moins de 8 millisecondes après écrémage des domaines pas $AC3$.

\subsection{Bonus}
La section bonus a été réalisée. Les pages relatives aux UE du site web \url{androide.lip6.fr} ont été copiées dans un fichier texte. C'est ce texte qui a servi de référence pour la création du dictionnaire. Il y a beaucoup de mots, les fréquences sont donc très petites. Ce dictionnaire a alors été modifié manuellement pour correspondre aux besoins. Vous avez pu voir le résultat sur la figure précédente (\ref{gridANDROIDE_dicoANDROIDE}). Cependant, si on l'applique au dictionnaire original et qu'on regarde la solution retournée par l'algorithme de Branch \& Bound, ce sont des mots très communs (et donc avec une fréquence plus élevée), qui est retournée :

\begin{figure}[H]
	\begin{center}
		\includegraphics{Images/gridANDROIDE_full_with_ANDROIDE.png}
		\caption{Grille gridANDROIDE résolue par Branch \& Bound -- dictionnaire valué ANDROIDE}
	\end{center}
\end{figure}

On peut remarquer la présence d'un mot qui a l'air de ne pas exister : \og UVRE \fg. Sa présence dans le dictionnaire est dû au fait que le mot \og \oe uvre \fg{}, lors de la création du dictionnaire, est transformé en \og uvre \fg{}. Cela est dû à la bibliothèque utilisée pour transformer les caractères accentués et spéciaux, qui ne sait pas par quoi remplacer le caractère \og \oe \fg{}.

\section*{Conclusion}
\addcontentsline{toc}{section}{Conclusion}
Les problèmes de CSP peuvent être appliqués à de nombreuses situations. Dans ce projet, les algorithmes de résolution tels que $RAC$, $CBJ$ et Branch \& Bound ont été utilisés pour résoudre des grilles de mots croisés à partir de dictionnaires, avec succès. Après de nombreux tests, il a été déterminé que pour les deux premiers algorithmes, c'est le nombre de variables présentes dans la grille, mais aussi et surtout le ratio entre le nombre de contraintes binaires entre les variables et le nombre de mots présents dans la grille. Plus ce ratio est élevé, plus le temps d'exécution sera lui aussi élevé. En revanche, pour le Branch \& Bound, c'est la taille du dictionnaire qui influera de manière exponentielle sur le temps d'exécution. Le nombre de mots présents dans la grille ne fera qu'augmenter de façon linéaire ce temps d'exécution.

Quant à la complexité de nos algorithmes, elle est exponentielle pour la plupart. Pour $RAC$ et $CBJ$, dans le pire des cas, pour chaque mot du domaine d'une variable, on va parcourir tous les mots de tous les domaines des autres variables avant de passer au second mot, réitérer l'opération, et ainsi de suite. Si l'on considère que $m$ est la somme des tailles du domaine de toutes les variables et on a k variables dans la grille, RAC et CBJ ont donc une complexité égal à $O(m^k)$ dans le pire des cas.
Quant à AC3, au pire, on retire un seul mot à chaque itération, et on effectue ensuite la même chose avec ce mot retiré. Cet algorithme est donc en $m^2$.

D'autre part, on remarque bien que le nombre de noeuds de l'arbre exhaustif des solutions possibles croit très vite lorsque la taille du dictionnaire utilisé et le nombre de variables $k$ augmentent (ce nombre est égal à $\mathlarger{\sum_{i = 0}^{n} \prod_{j = 0}^{i} |D(x_j)|}$), et qu'avec nos tests, on remarque bien que la performance de notre algorithme diffère selon l'instance. La méthode Branch \& Bound n'est qu'une construction et un parcours astucieux de cet arbre, qui permet de ne pas explorer certaines branches grâce à un système de minoration et majoration (bornes inférieures et supérieures) des solutions que cette branche peut produire. Sur certaines instances, les temps d'exécution dépendaient plus sur la position de la solution optimale dans l'arbre que sur le nombre de variables dans la grille ou de la taille du dictionnaire. En effet, si une solution se trouvait dans la première branche d'un arbre de grande taille, on pourrait obtenir un temps d'exécution inférieur au temps d'exécution du B\&B sur un arbre de petite taille dont la solution optimale se trouvait dans une des dernières branches de cette arbre. Ceci entraine le parcours de plus de branches de l'arbre, ce qui est tout simplement dû au fait que l'on n'ait pas de \og chance \fg{} sur l'ordre de parcours, et par conséquent, notre algorithme de Branch \& Bound explorera un très grand ou très petit nombre de noeuds avant de renvoyer la solution optimale quel que soit le nombre de variables et de mots dans les domaines.

Lors de l'implémentation de ces différents algorithmes, la compréhension de ceux-ci n'a pas été la difficulté principale rencontrée. En revanche, les subtilités de certaines méthodes du langage Python ont créé des retards inattendus. Par exemple, la méthode \og union \fg{} s'appliquant sur deux sets et effectuant leur union, n'effectue pas cette union en place, mais renvoie l'union de ces deux sets, sans modifier ni l'un ni l'autre. A cause de cette croyance erronée de notre part, de nombreuses heures de travail ont été perdues à scruter les exécutions dans les moindres détails pour comprendre d'où provenait le problème. Quoi qu'il en soit, tous les problèmes rencontrés ont été résolus, et une application fonctionnelle et complète a été crée.

\medskip
Pour ce projet il a aussi été nécessaire de réaliser une interface utilisateur pour permettre de lancer la résolution de grilles plus simplement. Le but premier était d'autant plus de permettre de visualiser cette résolution étape par étape, d'afficher toutes les tentatives d'instanciation et le retour en arrière éventuel lorsqu'il est effectué. Elle permet donc comme demandé d'ouvrir n'importe quel fichier contenant une grille au format qui a été conçu spécialement pour ce type de fichier, mais aussi de choisir quel dictionnaire utiliser pour la résolution de l'algorithme. D'ailleurs, le choix de l'algorithme à lancer, avec toutes les options souhaitées, est facilement indiqué par l'utilisateur par l'intermédiaire de boutons et \og checkboxes \fg{}.
Pour cela, c'est la bibliothèque Tkinter qui a été choisie. Elle permet d'ajouter relativement simplement des éléments à l'interface et à connecter ces éléments avec les fonctions, procédures et méthodes qui ont été implémentées. Cependant, implémenter l'affichage de la résolution des grilles à chaque tentative d'instanciation, ou l'effacement d'une de ces instanciation lors d'un retour arrière a été la plus grande difficulté rencontrée à la réalisation de cette interface. En effet, l'utilisation simple de threads était assez mal documentée sur les différents sites parcourus, en particulier pour la mise en pause volontaire d'un thread afin de permettre l'affichage au tour par tour de la part de l'utilisateur. Cependant, malgré quelques ralentissements possibles dus à la gestion anarchique des threads par le système d'exploitation, ces fonctionnalités ont été implémentées avec succès.
...
\begin{figure}[H]
	\begin{center}
		\includegraphics{Images/gridANDROIDE_empty.png}
		\includegraphics{Images/gridANDROIDE_full_with_ANDROIDE.png}
		\includegraphics{Images/gridANDROIDE_full_with_dicoANDROIDE.png}
		\caption{Grille gridANDROIDE résolue par Branch \& Bound}
	\end{center}
\end{figure}
...
\begin{center}
	\texttt{python3 run.py}
\end{center}

Le programme fonctionne uniquement en \emph{Python3}. Les bibliothèques nécessaires à certaines fonctionnalités n'étant pas disponible ou implémentée trop différemment en \emph{Python2}

\subsubsection{Pré-requis}
La bibliothèque emph{tkinter} est nécessaires à l'exécution du programme.
Elle sert à l'affichage des fenêtres.

Pour installer \emph{tkinter} sous \emph{Python3}, sur Linux/Debian tapez dans un terminal
\begin{center}
	\texttt{sudo apt-get install python-tk python-imaging-tk python3-tk tk8.6-dev}
\end{center}

Pour Windows et Mac, il est plus facile d'installer directement la dernière version de Python disponible à cette adresse : \url{https://www.python.org/downloads/}
